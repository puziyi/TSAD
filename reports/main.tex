...
Anomaly Detection in sequence data remains a challenging task due to its nonlinear nature. For most deep learning researchers, recurrent networks is commonly used in sequence modeling. Yet recent studies indicate that models with convolutional architectures can perform better than recurrent networks on tasks such as audio synthesis and machine translation. Then given a time series task or dataset, would convolutional architectures still works well? In our work, we reviewing and experimentally evaluating on a real-world electric load dataset by contrasting deep learning architectures on short term forecast. The algorithms we used in project are feedforward neural networks(FNN),  recurrent neural networks (RNN) with Long Short-Term Memory (LSTM) and Gated Recurrent Units (GRU), sequence to sequence models (seq2seq) and temporal convolutional neural networks (TCN). 
\end{abstract}
